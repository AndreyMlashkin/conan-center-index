...
from conan.files.symlinks import absolute_to_relative_symlinks
import os
...
class MdnsResponderConan(ConanFile):
    ...
    def _build_make(self):
        with files.chdir(self, self._posix_folder):
    ...
    def _build_msvc(self):
        ...
        if "MD" in self.settings.compiler.runtime:
            ...
            for vcxproj in [dll_vcxproj, dllstub_vcxproj, dns_sd_vcxproj]:
                files.replace_in_file(self, vcxproj, "<RuntimeLibrary>MultiThreaded</RuntimeLibrary>", "<RuntimeLibrary>MultiThreadedDLL</RuntimeLibrary>")
                files.replace_in_file(self, vcxproj, "<RuntimeLibrary>MultiThreadedDebug</RuntimeLibrary>", "<RuntimeLibrary>MultiThreadedDebugDLL</RuntimeLibrary>")
        ...
        for rc in [dll_rc, dns_sd_rc]:
            files.replace_in_file(self, rc, "afxres.h", "winres.h")
    ...
    def _install_make(self):
        for dir in ["bin", "include", "lib", "sbin"]:
            files.mkdir(self, os.path.join(self.package_folder, dir))
        with files.chdir(self, self._posix_folder):
            autotools = self._configure_autotools()
            autotools.make(args=self._make_install_args, target=self._make_install_targets)
        # package the daemon in bin too
        files.rename(self, os.path.join(self.package_folder, "sbin", "mdnsd"),
    ...
    def _install_msvc(self):
        ...
        files.rename(self, src=os.path.join(self.package_folder, "lib", "dnssdStatic.lib"),
