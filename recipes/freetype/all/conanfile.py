...
class FreetypeConan(ConanFile):
    ...
    def _patch_sources(self):
        ...
        files.replace_in_file(self, cmakelists, find_harfbuzz, "")
        files.replace_in_file(self, cmakelists, if_harfbuzz_found, "if(0)")
        if not self.options.with_png:
            files.replace_in_file(self, cmakelists, "find_package(PNG)", "")
            files.replace_in_file(self, cmakelists, "if (PNG_FOUND)", "if(0)")
        if not self.options.with_zlib:
            files.replace_in_file(self, cmakelists, "find_package(ZLIB)", "")
            files.replace_in_file(self, cmakelists, "if (ZLIB_FOUND)", "if(0)")
        if not self.options.with_bzip2:
            files.replace_in_file(self, cmakelists, "find_package(BZip2)", "")
            files.replace_in_file(self, cmakelists, "if (BZIP2_FOUND)", "if(0)")
        if self._has_with_brotli_option:
            ...
            if not self.options.with_brotli:
                files.replace_in_file(self, cmakelists, "find_package(BrotliDec)", "")
                files.replace_in_file(self, cmakelists, "if (BROTLIDEC_FOUND)", "if(0)")

        config_h = os.path.join(self._source_subfolder, "include", "freetype", "config", "ftoption.h")
        if self.options.subpixel:
            files.replace_in_file(self, config_h, "/* #define FT_CONFIG_OPTION_SUBPIXEL_RENDERING */", "#define FT_CONFIG_OPTION_SUBPIXEL_RENDERING")
    ...
    def _make_freetype_config(self, version):
        ...
        files.replace_in_file(self, freetype_config, r"%PKG_CONFIG%", r"/bin/false")  # never use pkg-config
        files.replace_in_file(self, freetype_config, r"%prefix%", r"$conan_prefix")
        files.replace_in_file(self, freetype_config, r"%exec_prefix%", r"$conan_exec_prefix")
        files.replace_in_file(self, freetype_config, r"%includedir%", r"$conan_includedir")
        files.replace_in_file(self, freetype_config, r"%libdir%", r"$conan_libdir")
        files.replace_in_file(self, freetype_config, r"%ft_version%", r"$conan_ftversion")
        files.replace_in_file(self, freetype_config, r"%LIBSSTATIC_CONFIG%", r"$conan_staticlibs")
        files.replace_in_file(self, freetype_config, r"-lfreetype", libs)
        files.replace_in_file(self, freetype_config, r"export LC_ALL", textwrap.dedent("""\
            export LC_ALL
            BINDIR=$(dirname $0)
    ...
    def _extract_libtool_version(self):
        conf_raw = files.load(self, os.path.join(self._source_subfolder, "builds", "unix", "configure.raw"))
    ...
    def package(self):
        ...
        files.save(self, self._libtool_version_txt, libtool_version)
    ...
    def package_info(self):
        ...
        libtool_version = files.load(self, self._libtool_version_txt).strip()
