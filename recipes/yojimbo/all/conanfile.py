...
class YojimboConan(ConanFile):
    ...
    def source(self):
        ...
        with open(submodule_filename, 'r') as submodule_stream:
            ...
            for path, submodule in submodules_data["submodules"][self.version].items():
                ...
                files.get(self, **submodule_data)
                submodule_source = os.path.join(self._source_subfolder, path)
                files.rmdir(self, submodule_source)
    ...
    def build(self):
        ...
        if self.settings.os == "Windows":
            ...
            files.replace_in_file(self, premake_path, "includedirs { \".\", \"./windows\"", "includedirs { \".\", %s" % include_path_str, strict=True)
            files.replace_in_file(self, premake_path, "libdirs { \"./windows\" }", "libdirs { %s }" % lib_path_str, strict=True)
            
            # Edit the premake script to change the name of libsodium
            files.replace_in_file(self, premake_path, "\"sodium\"", "\"libsodium\"", strict=True)
            
        else:
        
        	# Edit the premake script to use  conan rather than local dependencies
            files.replace_in_file(self, premake_path, "\"/usr/local/include\"", include_path_str, strict=True)
