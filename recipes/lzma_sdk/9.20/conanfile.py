...
class LzmaSdkConan(ConanFile):
        ...
        for make_dir, _ in self._msvc_build_dirs:
            with tools.vcvars(self):
                with tools.environment_append(VisualStudioBuildEnvironment(self).vars):
                    with files.chdir(self, make_dir):
    ...
    def _patch_sources(self):
        if self.settings.compiler == "Visual Studio":
            files.replace_in_file(self, os.path.join(self._source_subfolder, "CPP", "Build.mak"),
        ...
        files.replace_in_file(self, os.path.join(self._source_subfolder, "C", "Util", "7z", "makefile.gcc"),
                              "CFLAGS = ",
                              "CFLAGS = -fpermissive ")
        files.replace_in_file(self, os.path.join(self._source_subfolder, "C", "Util", "7z", "makefile.gcc"),
                              ": 7zAlloc.c",
                              ": ../../7zAlloc.c")
        files.replace_in_file(self, os.path.join(self._source_subfolder, "C", "Util", "Lzma", "makefile.gcc"),
                              "CFLAGS = ",
                              "CFLAGS = -fpermissive ")
        files.replace_in_file(self, os.path.join(self._source_subfolder, "CPP", "Common", "MyString.h"),
                              "#ifdef _WIN32\r\n",
                              "#ifdef _WIN32\r\n#ifndef UNDER_CE\r\n#include <windows.h>\r\n#endif\r\n")
