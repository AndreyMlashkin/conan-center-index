...
class GtsamConan(ConanFile):
    ...
    def validate(self):
        ...
        if is_msvc(self) and Version(self.settings.compiler.version) < 15:
            raise ConanInvalidConfiguration ("GTSAM requires MSVC >= 15")

        if is_msvc(self) and Version(self.version) >= '4.1' \
    ...
    def _patch_sources(self):
        ...
        if is_msvc(self):
            files.replace_in_file(self, os.path.join(self._source_subfolder, "cmake", "GtsamBuildTypes.cmake"),
                                  "/MD ",
