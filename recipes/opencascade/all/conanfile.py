...
class OpenCascadeConan(ConanFile):
    ...
    def _patch_sources(self):
        ...
        files.replace_in_file(self, cmakelists, "3RDPARTY_INCLUDE_DIRS", "CONAN_INCLUDE_DIRS")
        files.replace_in_file(self, cmakelists, "3RDPARTY_LIBRARY_DIRS", "CONAN_LIB_DIRS")
        files.replace_in_file(self, cmakelists_tools, "3RDPARTY_INCLUDE_DIRS", "CONAN_INCLUDE_DIRS")
        files.replace_in_file(self, cmakelists_tools, "3RDPARTY_LIBRARY_DIRS", "CONAN_LIB_DIRS")

        # Do not fail due to "fragile" upstream logic to find dependencies
        files.replace_in_file(self, cmakelists, "if (3RDPARTY_NOT_INCLUDED)", "if(0)")
        files.replace_in_file(self, cmakelists, "if (3RDPARTY_NO_LIBS)", "if(0)")
        files.replace_in_file(self, cmakelists, "if (3RDPARTY_NO_DLLS)", "if(0)")
        ...
        files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/freetype\")", "")
        files.replace_in_file(self, 
            occt_csf_cmake,
        ...
        files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/tcl\")", "")
        csf_tcl_libs = "set (CSF_TclLibs \"{}\")".format(" ".join(self.deps_cpp_info["tcl"].libs))
        files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclLibs     \"tcl86\")", csf_tcl_libs)
        files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclLibs   Tcl)", csf_tcl_libs)
        if Version(self.version) >= "7.6.0":
            files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclLibs   \"tcl8.6\")", csf_tcl_libs)
        else:
            files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclLibs     \"tcl8.6\")", csf_tcl_libs)
        ## tk
        if self._link_tk:
            conan_targets.append("CONAN_PKG::tk")
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/tk\")", "")
            csf_tk_libs = "set (CSF_TclTkLibs \"{}\")".format(" ".join(self.deps_cpp_info["tk"].libs))
            files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclTkLibs   \"tk86\")", csf_tk_libs)
            files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclTkLibs Tk)", csf_tk_libs)
            if Version(self.version) >= "7.6.0":
                files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclTkLibs \"tk8.6\")", csf_tk_libs)
            else:
                files.replace_in_file(self, occt_csf_cmake, "set (CSF_TclTkLibs   \"tk8.6\")", csf_tk_libs)
        ## fontconfig
        if self._is_linux:
        ...
        if self.options.with_tbb:
            ...
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/tbb\")", "")
            files.replace_in_file(self, 
                occt_csf_cmake,
        ...
        if self.options.with_ffmpeg:
            ...
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/ffmpeg\")", "")
            files.replace_in_file(self, 
                occt_csf_cmake,
        ...
        if self.options.with_freeimage:
            ...
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/freeimage\")", "")
            files.replace_in_file(self, 
                occt_csf_cmake,
        ...
        if self.options.with_openvr:
            ...
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/openvr\")", "")
            files.replace_in_file(self, 
                occt_csf_cmake,
        ...
        if self.options.with_rapidjson:
            ...
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/rapidjson\")", "")
        ## draco
        if self.options.get_safe("with_draco"):
            conan_targets.append("CONAN_PKG::draco")
            files.replace_in_file(self, cmakelists, "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/draco\")", "")
        ## opengl
        files.replace_in_file(self, 
            occt_csf_cmake,
            "set (CSF_OpenGlLibs ",
        ...
        files.replace_in_file(self, occt_defs_flags_cmake, "-fPIC", "")
        files.replace_in_file(self, occt_defs_flags_cmake, "-std=c++0x", "")
        files.replace_in_file(self, occt_defs_flags_cmake, "-std=gnu++0x", "")
        files.replace_in_file(self, occt_defs_flags_cmake, "-stdlib=libc++", "")
        files.replace_in_file(self, occt_csf_cmake,
...
#pragma comment (lib, "tk"  STRINGIZE2(TCL_MAJOR_VERSION) STRINGIZE2(TCL_MINOR_VERSION) ".lib")""",
    ...
    def _replace_package_folder(self, source, target):
        if os.path.isdir(os.path.join(self.package_folder, source)):
            files.rmdir(self, os.path.join(self.package_folder, target))
    ...
    def _get_modules_from_source_code(self):
        ...
        modules_content = files.load(self, os.path.join(self.build_folder, self._source_subfolder, "adm", "MODULES"))
        packaged_libs_list = files.collect_libs(self, self, "lib")
        for module_line in modules_content.splitlines():
            ...
            for component_name in components_list:
                ...
                externlib_content = files.load(self, os.path.join(self.build_folder, self._source_subfolder,
    ...
    def _create_modules_json_file(self, modules):
        files.save(self, self._modules_helper_filepath, json.dumps(modules, indent=4))
    ...
    def package_info(self):
        ...
        occt_modules_json_content = files.load(self, self._modules_helper_filepath)
