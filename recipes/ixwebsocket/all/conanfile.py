...
class IXWebSocketConan(ConanFile):
    ...
    def validate(self):
        if self.settings.compiler.get_safe("cppstd"):
            ...
            tools.build.check_min_cppstd(self, 14 if Version(self.version) < "11.0.8" else 11)
    ...
    def _patch_sources(self):
        ...
        files.replace_in_file(self, cmakelists, "MBEDTLS_INCLUDE_DIRS", "MbedTLS_INCLUDE_DIRS")
        files.replace_in_file(self, cmakelists, "MBEDTLS_LIBRARIES", "MbedTLS_LIBRARIES")
        # Do not force PIC
        if Version(self.version) >= "9.5.7":
            files.replace_in_file(self, cmakelists, "set(CMAKE_POSITION_INDEPENDENT_CODE ON)", "")
        # Allow shared
        if Version(self.version) < "11.1.4":
            files.replace_in_file(self, cmakelists, "add_library( ixwebsocket STATIC", "add_library( ixwebsocket")
