...
class OpenCVConan(ConanFile):
    ...
    def _patch_opencv(self):
        ...
        for directory in ['libjasper', 'libjpeg-turbo', 'libjpeg', 'libpng', 'libtiff', 'libwebp', 'openexr', 'protobuf', 'zlib', 'quirc']:
            files.rmdir(self, os.path.join(self._source_subfolder, '3rdparty', directory))
        if self.options.with_openexr:
        ...
        files.replace_in_file(self, os.path.join(self._source_subfolder, "CMakeLists.txt"), "elseif(EMSCRIPTEN)", "elseif(QNXNTO)\nelseif(EMSCRIPTEN)")
        ...
        if self.options.dnn:
            ...
            files.replace_in_file(self, find_protobuf,
        ...
        if self.options.with_ade:
            ...
            if(ade_DIR)'''
            files.replace_in_file(self, ade_cmake, 'if(ade_DIR)', replacement, strict=False)
            files.replace_in_file(self, ade_cmake, 'if (ade_DIR)', replacement, strict=False)
            files.replace_in_file(self, ade_cmake, "TARGET ade", "TARGET ade::ade")
            gapi_cmake = os.path.join(self._source_subfolder, "modules", "gapi", "CMakeLists.txt")
            files.replace_in_file(self, gapi_cmake, " ade)", " ade::ade)")
    ...
    def package(self):
        ...
        if os.path.isfile(os.path.join(self.package_folder, "setup_vars_opencv4.cmd")):
            files.rename(self, os.path.join(self.package_folder, "setup_vars_opencv4.cmd"),
                ...
                if self.settings.os == "Android":
                    ...
                    if not self.options.shared:
                        ...
                        if conan_component == "opencv_core":
                            ...
                            self.cpp_info.components[conan_component].libs += files.collect_libs(self, self)

                if self.settings.os in ["iOS", "Macos", "Linux", "Neutrino"]:
                    if not self.options.shared:
                        if conan_component == "opencv_core":
                            libs = list(filter(lambda x: not x.startswith("opencv"), files.collect_libs(self, self)))
