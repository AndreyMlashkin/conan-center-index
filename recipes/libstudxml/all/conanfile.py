...
class LibStudXmlConan(ConanFile):
    ...
    def _build_vs(self):
        ...
        if not self.options.shared:
            files.replace_in_file(self, proj_path, "DynamicLibrary", "StaticLibrary")
            files.replace_in_file(self, proj_path, "LIBSTUDXML_DYNAMIC_LIB", "LIBSTUDXML_STATIC_LIB")
    ...
    def _build_autotools(self):
        ...
        if self.settings.compiler.get_safe("libcxx") == "libc++":
            ...
            files.rm(self, "version", self._source_subfolder)
    ...
    def package(self):
        ...
        if self.settings.compiler == "Visual Studio":
            ...
            autotools = self._configure_autotools()
            autotools.install()
            files.rm(self, "libstudxml.la", os.path.join(self.package_folder, "lib"))
